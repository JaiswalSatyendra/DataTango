...
def get_questions(user_id: str, survey_id: str):
    ...
    cache_key = f"conversations-{user_id}-{survey_id}"
...
async def generateText(user_id: str, survey_id: str, request: Request) -> JSONResponse:
    ...
    cache_key = f"responses-{user_id}-{survey_id}-{conversation_key}"
    if cache_key in response_cache:
    ...
    response_cache[cache_key] = response
    response_cache[f"conversations-{user_id}-{survey_id}"] = {"conversations": conversations}
